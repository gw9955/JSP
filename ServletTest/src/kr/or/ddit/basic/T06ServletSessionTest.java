 ...
 * 	3. web.xml에 ＜session-config＞ 설정하기 (분단위로 설정함.)
 */
public class T06ServletSessionTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session.isNew()) { // 세션객체가 새로 만들어진 경우.
			...
			visitCnt = (Integer) session.getAttribute("visitCnt"); // 세션에 있는 기존 값
			visitCnt++;
			userId = (String) session.getAttribute("userId");
		}

		session.setAttribute("visitCnt", visitCnt); // 세션에 ++된 방문수 저장
		
		// 세션 삭제
//		session.invalidate(); // 1번 방법
		session.setMaxInactiveInterval(10); // 2번 방법: 20초
