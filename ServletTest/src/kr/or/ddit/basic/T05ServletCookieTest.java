...
public class T05ServletCookieTest extends HttpServlet {
	    ...
	    *     - 값
	    *     - 유호시간(초)
	    *     - 도메인 : ex) www.somehos.com, .somehost.com
...
//		setCoookieExam(req, resp); // 쿠키 설정 예제
...
//		readCookieExam(req, resp); // 쿠키정보 읽기 예제
	...
	private void deleteCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (cookies != null) {
			...
			for (Cookie cookie : cookies) {
				...
				out.println("value : " + URLDecoder.decode(cookie.getValue()) + "<br>");
				out.println("<hr>");
			}
		} else {
	...
	private void setCoookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		/*
		 * 쿠키정보 설정하는 방법 1. 쿠키객체 생성하기. 사용불가문자(공백, []()=, "/?@:;) Cookie cookie = new
		 * Cookie("키값", "value값"); 쿠키값은 사용불가문자를 제외한 나머지 출력가능한 아스키 문자 사용가능함. => 이외의
		 * 값(예를들면 한글)을 사용시에는 URLEncoder.encode() 사용하여 처리함.
		 * 
		...
		out.println("<body>" + "<h1 align= \"center\">" + title + "</h1>" + "<ul>" + "<li><b>ID</b>: "
				+ req.getParameter("userId") + "</li>" + "<li><b>이름</b>:" + req.getParameter("name") + "</li>" + "</ul>"
				+ "</body>" + "</html>");
	}
