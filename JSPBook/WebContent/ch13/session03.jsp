...
<!-- session기본(내장) 객체
	...
	5. 	1) 웹 서버(톰켓)는 웹 브라우저(크롬)에 세션ID를 전송
		2) 웹 브라우저는 웹 서버에 요청시 마다 매번 세션ID를 보내서(쿠키에 넣어서)
			웹 서버(톰켓)가 어떤 세션(웹 브라우저)를 사용할지 판단할 수 있게 한다
	6. 웹 서버(톰켓)는 세션ID를 이용하여 웹 브라우저를 위한 세션을 찾는다
		세션ID를 공유하기 위해 쿠키를 사용한다(JSESSIONID)
		JSESSIONID 쿠기가 세션ID를 공유할 떄 사용하는 쿠키이다
-->
	...
	<%
		...
		String value;
		// 세션 객체에 담겨있는 모든 속성 명을 열거형 타입으로 가져온다
